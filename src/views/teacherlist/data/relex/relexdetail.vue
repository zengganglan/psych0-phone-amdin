    ...
    return {
      ...
      tableData: [],
       data:[{
          ...
          job_num:2,
          name:'徐媛',
          sand_file_name:"李静.xlsx",
          upload_time:'2019-04-24 18:51:15'
  ...
  methods: {
    ...
    handleDownload() {
      ...
      if (this.multipleSelection.length !== 0) {
        ...
        for (var i = 0; i < ids.length; i++) {
          this.axios
            .post("/api/v1/admin/sand/file/download/" + ids[i], config)
            .then(function(res) {
    ...
    removeExcel() {
      ...
      if (this.elfileId) {
        this.axios
          .post(
            "/api/v1/admin/sand/file/move/" + this.fileId + "/" + this.elfileId
          )
          .then(function(res) {
            if (res["data"]["code"] == 0) {
              that.getlist(that.page.currentpage, that.page.pagesize);
