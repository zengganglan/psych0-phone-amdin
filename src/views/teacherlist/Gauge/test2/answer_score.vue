  ...
  <div>
    ...
    <table class="am-table">
      <thead>
        ...
        <th>测评时间</th>
        <th v-for="(item1,index1) in scalelist[0].answer" :key="index1">{{index1+1}}</th>
      </thead>
      <tbody>
        <tr v-for="(item,index) in scalelist" :key="index">
          ...
          <td v-for="(item1,index1) in item.answer" :key="index1">{{item1}}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  created() {
    ...
    this.scalelist.map((item, index) => {
      ...
      var result = JSON.parse(item["answer"]);
  ...
  methods: {
    Download() {
     ...
     this.scalelist[0].answer.map((item,index)=>{
        filterVal.push(index+1)
        tHeader.push(index+1)
